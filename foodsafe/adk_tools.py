...
def _ok(payload: dict[str, Any], provenance: Any = None) -> dict[str, Any]:
    result = {"status": "success", **payload}
    if provenance is not None:
        result["provenance"] = provenance.as_dict()
    return result
...
def resolve_protein(name: str, organism: str) -> dict:
    ...
    return _ok({"protein": found.value.as_dict()}, found.provenance)
...
def fetch_protein_structure(uniprot_accession: str) -> dict:
    ...
    return _ok({"structure": found.value.as_dict()}, found.provenance)
...
def resolve_compound(name: str) -> dict:
    ...
    return _ok({"compound": found.value.as_dict()}, found.provenance)
...
def describe_molecule(smiles: str) -> dict:
    ...
    return _ok({"descriptors": found.value.as_dict()}, found.provenance)
...
def search_literature(protein: str, contaminant: str) -> dict:
    ...
    return _ok(
        {
            "citation_count": len(found.value),
            "citations": [c.as_dict() for c in found.value],
        },
        found.provenance,
    )
    ...
    return _ok(
        {
            ...
            "statements": [c.statement() for c in comparisons],
        }
    )
